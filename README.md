...
2. **AgentOS UI.** Chat with agents, build new ones, inspect sessions, traces, memory, and evals from the AgentOS UI at [os.agno.com](https://os.agno.com?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-fly&utm_content=agentos-fly&utm_term=fly).
...
You can run the platform anywhere that supports containerized images. This codebase comes with scripts to deploy the platform to [Fly.io](https://fly.io) — and a coding-agent skill, [`/deploy-platform`](.agents/skills/deploy-platform/SKILL.md), that drives them for you and verifies the live platform at the end.
...
1. Open [os.agno.com](https://os.agno.com?utm_source=github&utm_medium=example-repo&utm_campaign=agentos-fly&utm_content=agentos-fly&utm_term=fly), click **Connect OS** → **Live**, and enter your Fly URL.
